...
*"Prior laundering: priors trained on legacy reconstructions inherit undetectable
overconfidence."*
...
**Not machine-checked (stated in the paper; proofs in the appendix).** After waves 1-3 the remainder
is short:
...
* `p:recover`(ii)'s **matrix-to-diagonal whitening reduction** (the diagonalized recursion and the
  matrix floor are both checked; the congruence between them is the appendix's).
